...
exports.giveawayEndCommand = exports.giveawayListCommand = exports.giveawayCreateCommand = void 0;
exports.setGiveawayDB = setGiveawayDB;
exports.checkGiveaways = checkGiveaways;
const discord_js_1 = require("discord.js");
const logger_1 = require("../utils/logger");
let db = null;
function setGiveawayDB(database) {
    db = database;
}
// Создание розыгрыша
async function createGiveaway(interaction, prize, winnersCount, durationHours, description) {
    if (!db)
        throw new Error('Database not initialized');
    const now = new Date();
    const endAt = new Date(now.getTime() + durationHours * 60 * 60 * 1000);
    const result = await db.run(`INSERT INTO giveaways (prize, winners_count, description, end_at, created_at) 
     VALUES (?, ?, ?, ?, ?)`, [prize, winnersCount, description || null, endAt.toISOString(), now.toISOString()]);
...
async function getGiveaway(giveawayId) {
    if (!db)
        return null;
    return await db.get(`SELECT * FROM giveaways WHERE id = ?`, [giveawayId]);
}
// Обновление статуса розыгрыша
async function updateGiveawayStatus(giveawayId, ended) {
    if (!db)
        return;
    await db.run(`UPDATE giveaways SET ended = ? WHERE id = ?`, [ended ? 1 : 0, giveawayId]);
}
...
async function checkGiveaways(client) {
    ...
    try {
        const now = new Date();
        const endedGiveaways = await db.all(`SELECT * FROM giveaways WHERE ended = 0 AND end_at <= ?`, [now.toISOString()]);
...
async function endGiveaway(client, giveaway) {
    ...
    try {
        ...
        if (channel && channel.isTextBased()) {
            ...
            if (message) {
                const endedContainer = new discord_js_1.ContainerBuilder()
                    .setAccentColor(0xFFD700)
                    .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent('🏆 **РОЗЫГРЫШ ЗАВЕРШЕН** 🏆'), new discord_js_1.TextDisplayBuilder().setContent(`🎁 **${giveaway.prize}**`), new discord_js_1.TextDisplayBuilder().setContent(`👑 Победители: ${winners.length > 0 ? winners.map((w) => `<@${w}>`).join(', ') : 'Нет участников'}`));
                await message.edit({
                    components: [endedContainer],
                    flags: discord_js_1.MessageFlags.IsComponentsV2
                });
            }
            logger_1.logger.info(`Розыгрыш #${giveaway.id} завершен. Победители: ${winners.join(', ')}`);
        }
    }
    catch (error) {
...
function buildGiveawayContainer(prize, winnersCount, endAt, description) {
    ...
    const container = new discord_js_1.ContainerBuilder()
        ...
        .addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent('🎁 **РОЗЫГРЫШ** 🎁'), new discord_js_1.TextDisplayBuilder().setContent(`**Приз:** ${prize}`), new discord_js_1.TextDisplayBuilder().setContent(`**Количество победителей:** ${winnersCount}`))
    ...
    if (description) {
        container.addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(`**Описание:**\n${description}`));
    ...
    container.addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(`⏰ **Осталось времени:** ${hours}ч ${minutes}м`), new discord_js_1.TextDisplayBuilder().setContent('🎉 **Нажмите на реакцию 🎉, чтобы участвовать!**'));
    return container;
}
// ------ КОМАНДЫ ------
// Создание розыгрыша
exports.giveawayCreateCommand = {
    ...
    async execute(interaction) {
        try {
            ...
            const { giveawayId, endAt } = await createGiveaway(interaction, prize, winnersCount, durationHours, description || '');
            // Создаем контейнер
            ...
            await db.run(`UPDATE giveaways SET channel_id = ?, message_id = ? WHERE id = ?`, [channel.id, message.id, giveawayId]);
            await interaction.reply({
                content: `✅ Розыгрыш **"${prize}"** создан! Участвуйте, нажимая на реакцию 🎉`,
                ephemeral: true
            });
            logger_1.logger.info(`Розыгрыш #${giveawayId} создан: ${prize}`);
        }
        catch (error) {
...
exports.giveawayListCommand = {
    data: new discord_js_1.SlashCommandBuilder()
        .setName('розыгрыш-список')
        .setDescription('Показать список активных розыгрышей'),
    async execute(interaction) {
        try {
            if (!interaction.isChatInputCommand())
                return;
            ...
            const giveaways = await db.all(`SELECT * FROM giveaways WHERE ended = 0 ORDER BY end_at ASC`);
