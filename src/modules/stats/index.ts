import { GuildMember, VoiceState, Message, EmbedBuilder } from 'discord.js';
import { Database } from 'sqlite';
import { logger } from '../../utils/logger';

class StatsModule {
  private db: Database | null = null;
  private voiceTimers: Map<string, number> = new Map();
  private readonly weekDays = 7;
  private botStartTime: Date = new Date();
  ...
  public async handleMessage(message: Message): Promise<void> {
    if (!this.db || message.author.bot) return;
  ...
  public async handleVoiceStateUpdate(oldState: VoiceState, newState: VoiceState): Promise<void> {
    ...
    if (!oldState.channelId && newState.channelId) {
      ...
      logger.info(`🎤 ${newState.member?.user.username} зашел в голосовой канал`);
    }
    ...
    if (oldState.channelId && !newState.channelId) {
      ...
      if (startTime) {
        ...
        if (durationMinutes > 0) {
          logger.info(`🎤 ${oldState.member?.user.username} вышел из голосового, пробыл ${durationMinutes} минут`);
          await this.addVoiceTime(userId, durationMinutes);
        } else {
          logger.info(`🎤 ${oldState.member?.user.username} вышел из голосового, пробыл меньше минуты`);
        }
  ...
  private async addVoiceTime(userId: string, minutes: number): Promise<void> {
    if (!this.db) return;
    
    try {
      const now = new Date();
      
      logger.info(`💾 Сохраняем ${minutes} минут для пользователя ${userId}`);
      ...
      const check = await this.db.get(
        `SELECT total_voice_minutes FROM user_stats WHERE user_id = ?`,
        [userId]
      );
      logger.info(`✅ После сохранения: у пользователя ${userId} ${check?.total_voice_minutes || 0} минут`);
      
    } catch (error) {
      logger.error('Error updating voice stats:', error);
    }
  }

  // Обновление недельной статистики
  private async updateWeeklyStats(userId: string, type: 'messages' | 'voice', amount: number): Promise<void> {
    if (!this.db) return;
    ...
    try {
      ...
      if (existing) {
        ...
        await this.db.run(
          `UPDATE weekly_stats SET ${field} = ${field} + ? WHERE id = ?`,
          [amount, existing.id]
        );
      } else {
        await this.db.run(
          `INSERT INTO weekly_stats (user_id, messages, voice_minutes, week_start, week_end)
           VALUES (?, ?, ?, ?, ?)`,
          [userId, type === 'messages' ? amount : 0, type === 'voice' ? amount : 0, weekStart.toISOString(), weekEnd.toISOString()]
        );
      }
    } catch (error) {
  ...
  public async getUserStats(userId: string): Promise<any> {
    ...
    try {
      ...
      const weekly = await this.db.get(
        `SELECT * FROM weekly_stats WHERE user_id = ? ORDER BY week_start DESC LIMIT 1`,
        [userId]
      );
  ...
  public async getTopUsersByMessages(limit: number = 10): Promise<any[]> {
    if (!this.db) return [];
  ...
  public async getTopUsersByVoice(limit: number = 10): Promise<any[]> {
    if (!this.db) return [];
    
    try {
      return await this.db.all(
        `SELECT user_id, total_voice_minutes, total_messages 
         FROM user_stats 
      ...
      logger.error('Error getting top voice users:', error);
      return [];
    }
  }

  // Создание embed со статистикой
  public async createStatsEmbed(userId: string): Promise<EmbedBuilder | null> {
  ...
  public async createTopEmbed(type: 'messages' | 'voice'): Promise<EmbedBuilder | null> {
    if (!this.db) return null;
    ...
    try {
      ...
      logger.error('Error creating top embed:', error);
      return null;
    }
  }

  // Проверка на неактивность (только через 7 дней после запуска бота)
  public async checkInactiveUsers(guildId: string, familyRoleId: string, inactiveRoleId: string): Promise<void> {
    if (!this.db) return;
    ...
    if (daysSinceStart < 7) {
      logger.info(`⏳ Бот работает ${daysSinceStart} дней. Проверка неактивных начнется через ${7 - daysSinceStart} дней.`);
      return;
    }
    ...
    try {
      const inactiveUsers = await this.db.all(
        `SELECT user_id FROM user_stats 
         ...
         AND weekly_voice_minutes = 0`,
        [weekAgo.toISOString()]
      );
      
      logger.info(`📊 Найдено ${inactiveUsers.length} неактивных пользователей`);
        ...
        if (member) {
          if (member.roles.cache.has(familyRoleId)) {
            try {
              ...
              assignedCount++;
              logger.info(`👤 Пользователю ${member.user.tag} выдана роль "Инактив"`);
            } catch (error) {
              logger.error(`Ошибка выдачи роли пользователю ${member.user.tag}:`, error);
            }
          }
        }
      }
      
      logger.info(`✅ Роль "Инактив" выдана ${assignedCount} пользователям`);
      
      await this.db.run(
        `UPDATE user_stats SET weekly_messages = 0, weekly_voice_minutes = 0`
      );
